...
# and a sentinel prompt attribute, then asserts the debug exporter's structured
# log line keeps the request_id and shape-only fields but drops the prompt
# content. No mocks: a real collector, real OTLP, the cluster's real config.
set -euo pipefail
...
assert_present() {
  if ! grep -qF "$1" <<<"${logs}"; then
    echo "FAIL: expected '$1' on the exporter log line, not found" >&2
    fail=1
  fi
}
assert_absent() {
  if grep -qF "$1" <<<"${logs}"; then
    echo "FAIL: '$1' leaked to the exporter log line (shape-only violated)" >&2
    fail=1
  fi
}

# request_id present on the log line (acceptance criterion 2).
assert_present "${request_id}"
# Shape-only fields survive redaction.
