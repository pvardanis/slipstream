...
1. **What drives `cluster-up`/`cluster-down`.** The spec and glossary describe a `cluster-up`/`cluster-down` verb
...
**Task runner: `just`.** A `justfile` at the repo root exposes `just cluster-up` and
`just cluster-down`.

- `cluster-up`: `terraform init` → `apply -auto-approve` → `aws eks update-kubeconfig` →
  `kubectl get nodes` smoke check.
- `cluster-down`: `terraform destroy -auto-approve`.
...
before the first `just cluster-up`. The main `terraform/eks/` stack uses the S3 backend
...
- `just cluster-down` returns cluster spend to zero; the bootstrap bucket is deliberately
